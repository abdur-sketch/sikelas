import { audit, ensureDatabase, getActor, requireAdmin, requireClassAccess, requireWriteRole, todayJakarta } from "../../../db/runtime";
...
export async function GET(request: Request) {
  try {
    ...
    const [classes, students, attendance, attendanceHistory, tasks, schedules, grades, settings, announcements, portfolios, pointRules, auditLogs, notifications, periods, submissions, communications, deletedRecords, users] = await Promise.all([
      ...
      actor.role==="Admin"?db.prepare("SELECT u.email,u.name,u.role,u.class_label AS classLabel,u.student_nis AS studentNis,u.status,u.active,u.approved_by AS approvedBy,u.created_at AS createdAt,u.updated_at AS updatedAt,GROUP_CONCAT(a.class_label) AS assignedClasses FROM users u LEFT JOIN user_class_assignments a ON a.email=u.email GROUP BY u.email ORDER BY CASE u.role WHEN 'Admin' THEN 1 WHEN 'Wali Kelas' THEN 2 WHEN 'Guru' THEN 3 ELSE 4 END,u.name").all():Promise.resolve({results:[]}),
    ]);
    const visibleClasses=actor.role==="Admin"?classes.results:classes.results.filter((item)=>assignedClasses.has(String(item.label)));
    return Response.json({ actor, classLabel, attendanceDate, rangeFrom, rangeTo, classes: visibleClasses.map(row), students: students.results.map(row), attendance: attendance.results.map(row), attendanceHistory: attendanceHistory.results.map(row), tasks: tasks.results.map(row), schedules: schedules.results.map(row), grades: grades.results.map(row), settings: settings ?? null, announcements: announcements.results.map(row), portfolios: portfolios.results.map(row), pointRules: pointRules.results.map(row), auditLogs: auditLogs.results.map(row), notifications: notifications.results.map(row), periods:periods.results.map(row), submissions:submissions.results.map(row), communications:communications.results.map(row), deletedRecords:deletedRecords.results.map(row),users:users.results.map(row) });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    if (resource === "task") {
      ...
      const existing=await db.prepare("SELECT * FROM tasks WHERE id=?").bind(taskId).first<Record<string,unknown>>();requireClassAccess(actor,String(data.classLabel||existing?.class_label||""));
    ...
    if (resource === "schedule") {
      ...
      const existing=await db.prepare("SELECT * FROM schedules WHERE id=?").bind(scheduleId).first<Record<string,unknown>>();requireClassAccess(actor,String(data.classLabel||existing?.class_label||""));
    ...
    if(resource==="submission"){
      ...
      const task=await db.prepare("SELECT class_label AS classLabel FROM tasks WHERE id=?").bind(taskId).first<{classLabel:string}>();if(!task)return Response.json({error:"Tugas tidak ditemukan."},{status:404});requireClassAccess(actor,task.classLabel);
    ...
    if(resource==="userManagement"){
      requireAdmin(actor);const email=String(data.email||"").trim().toLowerCase(),name=String(data.name||"").trim(),role=String(data.role||"Guru"),status=String(data.status||"Active");const classLabels=Array.isArray(data.classLabels)?data.classLabels.map(String).filter(Boolean):[];
      if(!email||!name||!["Admin","Wali Kelas","Guru"].includes(role)||!["Active","Pending","Nonaktif"].includes(status))return Response.json({error:"Data pengguna tidak valid."},{status:400});if(role!=="Admin"&&!classLabels.length)return Response.json({error:"Guru atau Wali Kelas wajib ditugaskan minimal ke satu kelas."},{status:400});if(email===actor.email&&(status!=="Active"||role!=="Admin"))return Response.json({error:"Admin Pusat tidak dapat menonaktifkan atau menurunkan perannya sendiri."},{status:400});
      ...
      await audit(db,actor,"MANAGE","user",email,`${role} · ${status} · ${classLabels.join(", ")||"tanpa kelas"}`);return Response.json({ok:true,email});
    }
