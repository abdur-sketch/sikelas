...
export async function GET(request: Request) {
  try {
    ...
    const metadata=await db.prepare("SELECT nis,evidence_name AS name,evidence_type AS type FROM portfolios WHERE evidence_key=?").bind(key).first<{nis:string;name:string;type:string}>();
