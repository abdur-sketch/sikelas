...
type AppActor = { email: string; name: string; role: string; classLabel: string | null; studentNis?: string | null;status?:string;active?:number;assignedClasses?:string[] };
type ClassRow = { id:string;label:string;short:string;boys:number;girls:number;students:number };
type UserRow = { email:string;name:string;role:string;classLabel:string|null;studentNis:string|null;status:string;active:number;approvedBy:string|null;createdAt:string;updatedAt:string|null;assignedClasses:string|null };
...
function TasksPage({ activeClass, studentRows, taskRows, gradeRows, refresh }: { activeClass: (typeof classOptions)[number]; studentRows: StudentRow[]; taskRows: BootstrapData["tasks"]; gradeRows: GradeRow[]; refresh: () => Promise<void> }) {
  ...
  const editTask=(task:BootstrapData["tasks"][number])=>{const title=window.prompt("Judul tugas",task.title)?.trim();if(!title)return;const subject=window.prompt("Mata pelajaran",task.subject)?.trim();if(!subject)return;const due=window.prompt("Batas pengumpulan (YYYY-MM-DD)",task.due)?.trim();if(!due||!/^\d{4}-\d{2}-\d{2}$/.test(due)){window.alert("Format tanggal harus YYYY-MM-DD.");return}return request({resource:"task",operation:"update",data:{...task,title,subject,due,total:studentRows.length}})};
...
function AdministrationPage({ activeClass, data, refresh }: { activeClass:(typeof classOptions)[number];data:BootstrapData;refresh:()=>Promise<void> }) {
  const isAdmin=data.actor.role==="Admin";const tabs=isAdmin?["Pengguna","Semester","Rekap Absensi","Pengumpulan","Portal Siswa","Kenaikan & Data","Persetujuan","Komunikasi","Pemulihan"]:["Rekap Absensi","Pengumpulan","Komunikasi"];
  ...
  const saveUser=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);await request("userManagement",{email:form.get("email"),name:form.get("name"),role:form.get("role"),status:form.get("status"),classLabels:form.getAll("classLabels")});setEditingUser(null)};
  const updateUserStatus=(user:UserRow,status:string)=>request("userManagement",{email:user.email,name:user.name,role:user.role,status,classLabels:(user.assignedClasses||user.classLabel||"").split(",").filter(Boolean)});
