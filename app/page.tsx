...
type AppActor = { email: string; name: string; role: string; classLabel: string | null; studentNis?: string | null };
...
function AdministrationPage({ activeClass, data, refresh }: { activeClass:(typeof classOptions)[number];data:BootstrapData;refresh:()=>Promise<void> }) {
  const tabs=["Semester","Rekap Absensi","Pengumpulan","Portal Siswa","Kenaikan & Data","Persetujuan","Komunikasi","Pemulihan"];
...
export default function Home() {
  ...
  const isPortalViewer=["Siswa","Wali Santri"].includes(appData?.actor.role??"");const visibleNavItems=isPortalViewer?navItems.filter(item=>["dashboard","tugas","perkembangan"].includes(item.key)):navItems;const visibleSecondaryItems=isPortalViewer?secondaryNavItems.filter(item=>item.key==="portofolio"):secondaryNavItems;
