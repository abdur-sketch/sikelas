...
type PageKey = "dashboard" | "siswa" | "absensi" | "jadwal" | "tugas" | "perkembangan" | "pengumuman" | "portofolio" | "administrasi" | "pengaturan";
...
type AppActor = { email: string; name: string; role: string; classLabel: string | null; studentNis?: string | null;status?:string;active?:number;assignedClasses?:string[];assignedSubjects?:string[] };
type ClassRow = { id:string;label:string;short:string;boys:number;girls:number;students:number };
type UserRow = { email:string;name:string;role:string;classLabel:string|null;studentNis:string|null;status:string;active:number;approvedBy:string|null;createdAt:string;updatedAt:string|null;assignedClasses:string|null;assignedSubjects:string|null };
...
type SubjectGradeRow = GradeRow & { subject:string;updatedBy?:string|null;updatedAt?:string };
type AdminOverview = {students:number;activeTeachers:number;pendingUsers:number;violations:number;pendingPortfolios:number;classes:{label:string;students:number;present:number;activeTasks:number}[]};
...
type PeriodRow = { id:string;label:string;semester:string;startDate:string;endDate:string;active:number;closedAt:string|null };
type SubmissionRow = { id:string;taskId:string;taskTitle:string;nis:string;status:string;score:number|null;feedback:string;evidenceUrl:string|null;submittedAt:string };
type CommunicationRow = { id:string;nis:string;student:string;channel:string;message:string;status:string;actorEmail:string;createdAt:string };
type DeletedRow = { id:string;entity:string;entityId:string;payload:string;deletedBy:string;deletedAt:string };
type AttendanceHistoryRow = { nis:string;date:string;status:Attendance;method:string;scannedAt:string };
type BootstrapData = { actor: AppActor;classLabel:string;classes:ClassRow[];users:UserRow[];attendanceDate:string;students:StudentRow[];attendance:{nis:string;status:Attendance;method:string;scannedAt:string}[];attendanceHistory:AttendanceHistoryRow[];tasks:(typeof tasks[number]&{id?:string;classLabel?:string})[];schedules:ScheduleRow[];grades:GradeRow[];subjectGrades:SubjectGradeRow[];subjects:{subject:string}[];subjectCatalog:{classLabel:string;subject:string}[];developmentRecords:DevelopmentRecord[];adminOverview:AdminOverview|null;settings:SettingsData|null;announcements:typeof announcements;portfolios:PortfolioRow[];pointRules:{id:string;type:string;label:string;points:number;active:number}[];auditLogs:{id:string;actorEmail:string;action:string;entity:string;entityId:string;detail:string;createdAt:string}[];notifications:{id:string;title:string;message:string;read:number;createdAt:string}[];periods:PeriodRow[];submissions:SubmissionRow[];communications:CommunicationRow[];deletedRecords:DeletedRow[]};
...
const secondaryNavItems: { key: PageKey; label: string; icon: string }[] = [
  ...
  { key: "administrasi", label: "Administrasi", icon: "▦" },
...
const pageMeta: Record<PageKey, { eyebrow: string; title: string; subtitle: string }> = {
  ...
  administrasi: { eyebrow: "Operasional sekolah", title: "Administrasi Terpadu", subtitle: "Kelola semester, rekap, portal wali, komunikasi, dan pemulihan data XI DKV 1." },
...
function AdminSchoolOverview({overview,goTo}:{overview:AdminOverview;goTo:(page:PageKey)=>void}) {
  return <section className="admin-school-overview"><div className="overview-heading"><div><p className="section-kicker">ADMIN PUSAT</p><h2>Ringkasan Seluruh Sekolah</h2><span>Pantau operasional semua kelas dari satu layar.</span></div><button className="primary-button" onClick={()=>goTo("administrasi")}>Kelola Sekolah</button></div><div className="overview-stats"><article><span>♙</span><div><small>Siswa aktif</small><strong>{overview.students}</strong></div></article><article><span>♧</span><div><small>Guru aktif</small><strong>{overview.activeTeachers}</strong></div></article><article className={overview.pendingUsers?"warning":""}><span>◷</span><div><small>Akun menunggu</small><strong>{overview.pendingUsers}</strong></div></article><article className={overview.violations?"danger":""}><span>!</span><div><small>Pelanggaran bulan ini</small><strong>{overview.violations}</strong></div></article><article><span>◇</span><div><small>Portofolio ditinjau</small><strong>{overview.pendingPortfolios}</strong></div></article></div><article className="panel overview-classes"><div className="panel-head"><div><p className="section-kicker">STATUS KELAS</p><h2>Kehadiran & Tugas Aktif</h2></div></div><div className="overview-class-grid">{overview.classes.map(item=>{const rate=item.students?Math.round(item.present/item.students*100):0;return <div key={item.label}><span><strong>{item.label}</strong><small>{item.present}/{item.students} hadir hari ini</small></span><b>{rate}%</b><div className="progress"><i style={{width:`${rate}%`}}/></div><em>{item.activeTasks} tugas aktif</em></div>})}</div></article></section>;
}
...
function TasksPage({ activeClass, studentRows, taskRows, subjectGradeRows, subjects, actor, refresh }: { activeClass: (typeof classOptions)[number];studentRows:StudentRow[];taskRows:BootstrapData["tasks"];subjectGradeRows:SubjectGradeRow[];subjects:{subject:string}[];actor:AppActor;refresh:()=>Promise<void> }) {
  ...
  const editTask=(task:BootstrapData["tasks"][number])=>{const title=window.prompt("Judul tugas",task.title)?.trim();if(!title)return;const due=window.prompt("Batas pengumpulan (YYYY-MM-DD)",task.due)?.trim();if(!due||!/^\d{4}-\d{2}-\d{2}$/.test(due)){window.alert("Format tanggal harus YYYY-MM-DD.");return}return request({resource:"task",operation:"update",data:{...task,title,due,total:studentRows.length}})};
  const saveGrade=(grade:SubjectGradeRow,field:keyof Pick<GradeRow,"assignment"|"practice"|"exam"|"attitude">,value:string)=>request({resource:"subjectGrade",data:{...grade,subject:selectedSubject,[field]:field==="attitude"?value:Number(value),classLabel:activeClass.label}});
  const csvRows=studentRows.map(student=>{const grade=subjectGradeRows.find(item=>item.nis===student.nis&&item.subject===selectedSubject);return [student.nis,student.name,selectedSubject,grade?.assignment??0,grade?.practice??0,grade?.exam??0,grade?.attitude??"B",grade?.finalScore??0]});
  return <><div className="toolbar"><div className="segmented"><button className={tab==="tugas"?"active":""} onClick={()=>setTab("tugas")}>Daftar Tugas</button><button className={tab==="nilai"?"active":""} onClick={()=>setTab("nilai")}>Rekap Nilai</button></div>{tab==="tugas"&&<button className="primary-button" onClick={()=>setAdding(!adding)}>{adding?"Tutup Form":"＋ Tambah Tugas"}</button>}</div>{adding&&tab==="tugas"&&<form className="panel inline-form" onSubmit={addTask}><div className="form-grid"><label>Judul tugas<input name="title" required/></label><label>Mata pelajaran<select name="subject" required>{subjects.map(item=><option key={item.subject}>{item.subject}</option>)}</select></label><label>Batas pengumpulan<input name="due" type="date" required defaultValue={todayIso()}/></label></div><div className="form-actions"><button className="primary-button">Simpan Tugas</button></div></form>}{tab==="tugas"?<section className="task-grid">{taskRows.map(task=>{const submitted=Math.min(task.submitted,activeClass.students);return <article className="panel task-card" key={task.id??task.title}><div className="task-card-top"><span className={`task-badge large ${task.tone}`}>{task.subject.slice(0,2).toUpperCase()}</span><span className={`status-chip ${task.status==="Selesai"?"done":"active"}`}>{task.status}</span></div><small>{task.subject} · {activeClass.label}</small><h2>{task.title}</h2><p>Batas pengumpulan <strong>{formatIndonesianDate(task.due)}</strong></p><div className="progress-label"><span>Terkumpul</span><strong>{submitted}/{activeClass.students}</strong></div><div className="progress"><i style={{width:`${(submitted/activeClass.students)*100}%`}}/></div><div className="card-actions"><button className="secondary-button" onClick={()=>editTask(task)}>Edit</button><button className="secondary-button" onClick={()=>updateTask(task)}>{task.status==="Selesai"?"Buka Kembali":"Selesai"}</button><button className="danger-button" onClick={()=>task.id&&window.confirm("Hapus tugas ini?")&&updateTask(task,"delete")}>Hapus</button></div></article>})}</section>:<article className="panel data-panel"><div className="toolbar"><div><h2>Rekap Nilai {activeClass.label}</h2><p className="muted">Nilai tersimpan per mata pelajaran · akses {actor.role}.</p></div><label className="subject-filter">Mata pelajaran<select value={selectedSubject} onChange={event=>setSelectedSubject(event.target.value)}>{subjects.map(item=><option key={item.subject}>{item.subject}</option>)}</select></label><button className="secondary-button" onClick={()=>downloadCsv(`rekap-nilai-${activeClass.label}.csv`,[["NIS","Nama","Mata Pelajaran","Tugas","Praktik","Ujian","Sikap","Nilai Akhir"],...csvRows])}>⇩ Unduh Rekap</button></div><div className="table-scroll"><table className="editable-grades"><thead><tr><th>Siswa</th><th>Tugas</th><th>Praktik</th><th>Ujian</th><th>Sikap</th><th>Nilai Akhir</th></tr></thead><tbody>{studentRows.map(student=>{const grade=subjectGradeRows.find(item=>item.nis===student.nis&&item.subject===selectedSubject)??{id:"",nis:student.nis,classLabel:activeClass.label,subject:selectedSubject,assignment:0,practice:0,exam:0,attitude:"B",finalScore:0};return <tr key={student.nis}><td><div className="student-name"><span className={`avatar ${student.color}`}>{student.initials}</span><strong>{student.name}</strong></div></td>{(["assignment","practice","exam"] as const).map(field=><td key={field}><input aria-label={`${field} ${student.name}`} type="number" min="0" max="100" defaultValue={grade[field]} onBlur={event=>saveGrade(grade,field,event.target.value)}/></td>)}<td><select defaultValue={grade.attitude} onChange={event=>saveGrade(grade,"attitude",event.target.value)}><option>A</option><option>B</option><option>C</option><option>D</option></select></td><td><strong className="final-score">{grade.finalScore}</strong></td></tr>})}</tbody></table></div></article>}</>;
}
...
function StudentDevelopmentPage({ activeClass, studentRows, pointRules, auditLogs, refresh, settings, actor }: { activeClass: string; studentRows: StudentRow[]; pointRules: BootstrapData["pointRules"]; auditLogs: BootstrapData["auditLogs"]; refresh: () => Promise<void>;settings:SettingsData|null;actor:AppActor }) {
  ...
  useEffect(() => {
    let cancelled = false;
    fetch(`/api/development-records?class=${encodeURIComponent(activeClass)}`)
      .then(async (response) => {
  ...
  return <>
    ...
    <details className="panel governance-panel"><summary>Aturan Poin & Riwayat Perubahan</summary><div className="governance-grid"><section><h3>Aturan poin sekolah</h3>{pointRules.map((rule) => <label key={rule.id}><span><strong>{rule.label}</strong><small>{rule.type}</small></span><input aria-label={`Poin ${rule.label}`} type="number" defaultValue={rule.points} disabled={actor.role!=="Admin"} onBlur={(event) => actor.role==="Admin"&&updatePointRule(rule.id, Number(event.target.value), Boolean(rule.active))} /></label>)}</section><section><h3>Aktivitas terbaru</h3>{auditLogs.slice(0, 6).map((log) => <div className="audit-item" key={log.id}><span>{log.action}</span><div><strong>{log.detail}</strong><small>{log.actorEmail} · {new Date(log.createdAt).toLocaleString("id-ID")}</small></div></div>)}</section></div></details>
    ...
    {view === "rekap" ? <article className="panel data-panel development-table"><div className="toolbar"><label className="search-field"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cari nama atau NIS..." /></label><span className="settings-tag">Semester Ganjil 2026/2027</span></div><div className="table-scroll"><table><thead><tr><th>Siswa</th><th>Portofolio</th><th>Poin Prestasi</th><th>Poin Pelanggaran</th><th>Total Poin</th><th>Predikat</th><th></th></tr></thead><tbody>{filtered.map(({ student, summary }) => <tr key={student.nis}><td><div className="student-name"><span className={`avatar ${student.color}`}>{student.initials}</span><span><strong>{student.name}</strong><small>{student.nis}</small></span></div></td><td><b>{summary.portfolioCount}</b> karya <small>+{summary.portfolioPoints} poin</small></td><td><span className="point positive">+{summary.achievementPoints}</span></td><td><span className="point negative">-{summary.violationPoints}</span></td><td><strong className="total-point">{summary.totalPoints}</strong></td><td><span className={`predicate ${summary.totalPoints < 20 ? "warning" : ""}`}>{getDevelopmentPredicate(summary.totalPoints)}</span></td><td><button className="secondary-button" onClick={() => { setSelectedNis(student.nis); setView("laporan"); }}>Lihat Laporan</button></td></tr>)}</tbody></table></div></article> : <article className="panel semester-report">
      ...
      <footer className="report-footer"><div><strong>Catatan Wali Kelas</strong><p>{selectedSummary.totalPoints >= 50 ? "Pertahankan prestasi dan terus kembangkan potensi serta tanggung jawab." : selectedSummary.totalPoints >= 20 ? "Terus tingkatkan prestasi, kedisiplinan, dan konsistensi dalam berkarya." : "Diperlukan pendampingan dan pembinaan bersama wali kelas serta wali santri."}</p><span className="digital-signature">✓ Disahkan digital oleh {actor.name} · {new Intl.DateTimeFormat("id-ID",{dateStyle:"long",timeZone:"Asia/Jakarta"}).format(new Date())}</span></div><div><span>{formatIndonesianDate(todayIso())}</span><strong>{settings?.homeroom??actor.name}</strong><small>Wali Kelas</small></div></footer>
    </article>}
  </>;
}

function ReportVerificationQr({value}:{value:string}) {
  const ref=useRef<HTMLCanvasElement>(null);
  useEffect(()=>{let cancelled=false;import("@zxing/library").then(({BarcodeFormat,QRCodeWriter})=>{if(cancelled||!ref.current)return;const size=128,matrix=new QRCodeWriter().encode(value,BarcodeFormat.QR_CODE,size,size),context=ref.current.getContext("2d");if(!context)return;const pixels=context.createImageData(size,size);for(let y=0;y<size;y++)for(let x=0;x<size;x++){const offset=(y*size+x)*4,color=matrix.get(x,y)?0:255;pixels.data.set([color,color,color,255],offset)}context.putImageData(pixels,0,0)});return()=>{cancelled=true}},[value]);
  return <canvas ref={ref} width="128" height="128" aria-label="QR verifikasi laporan"/>;
}

function SemesterReportPanel({data,activeClass,selectedNis,onSelect}:{data:BootstrapData;activeClass:(typeof classOptions)[number];selectedNis:string;onSelect:(nis:string)=>void}) {
  const student=data.students.find(item=>item.nis===selectedNis)??data.students[0];if(!student)return <div className="empty-state"><strong>Belum ada siswa untuk dilaporkan.</strong></div>;
  const period=data.periods.find(item=>item.active)??data.periods[0];const records=data.developmentRecords.filter(item=>item.nis===student.nis);const grades=data.subjectGrades.filter(item=>item.nis===student.nis);const attendance=data.attendanceHistory.filter(item=>item.nis===student.nis);const present=attendance.filter(item=>item.status==="Hadir"||item.status==="Terlambat").length;const attendanceRate=attendance.length?Math.round(present/attendance.length*100):0;const average=grades.length?Math.round(grades.reduce((sum,item)=>sum+item.finalScore,0)/grades.length):0;const points=records.reduce((sum,item)=>sum+item.points,0);const verification=`SIKELAS-REPORT:${period?.id??"semester"}:${student.nis}:${activeClass.label}:${todayIso()}`;
  return <><div className="report-toolbar semester-report-toolbar"><label>Pilih siswa<select value={student.nis} onChange={event=>onSelect(event.target.value)}>{data.students.map(item=><option value={item.nis} key={item.nis}>{item.name} · {item.nis}</option>)}</select></label><button className="primary-button" onClick={()=>window.print()}>⎙ Cetak / Simpan PDF</button></div><article className="official-semester-report panel"><header><Logo/><div><h1>{data.settings?.schoolName??"SMK Nurul Iman"}</h1><p>{data.settings?.address??"Pesawaran, Lampung"}</p><small>NPSN {data.settings?.npsn??"-"} · {data.settings?.schoolEmail??"-"}</small></div><span>LAPORAN HASIL BELAJAR</span></header><section className="official-identity"><div><small>Nama peserta didik</small><strong>{student.name}</strong></div><div><small>NIS</small><strong>{student.nis}</strong></div><div><small>Kelas</small><strong>{activeClass.label}</strong></div><div><small>Semester</small><strong>{period?.semester??"Ganjil"}</strong></div><div><small>Tahun ajaran</small><strong>{period?.label??data.settings?.academicYear??"2026/2027"}</strong></div><div><small>Wali kelas</small><strong>{data.settings?.homeroom??data.actor.name}</strong></div></section><section className="official-summary"><div><small>Rata-rata nilai</small><strong>{average}</strong></div><div><small>Kehadiran</small><strong>{attendanceRate}%</strong></div><div><small>Total poin perkembangan</small><strong>{points>0?`+${points}`:points}</strong></div><div><small>Portofolio</small><strong>{data.portfolios.filter(item=>item.nis===student.nis).length}</strong></div></section><section><h2>A. Capaian Akademik per Mata Pelajaran</h2><table><thead><tr><th>Mata pelajaran</th><th>Tugas</th><th>Praktik</th><th>Ujian</th><th>Sikap</th><th>Nilai akhir</th><th>Predikat</th></tr></thead><tbody>{grades.map(item=><tr key={item.subject}><td>{item.subject}</td><td>{item.assignment}</td><td>{item.practice}</td><td>{item.exam}</td><td>{item.attitude}</td><td><strong>{item.finalScore}</strong></td><td>{item.finalScore>=90?"A":item.finalScore>=80?"B":item.finalScore>=70?"C":"D"}</td></tr>)}</tbody></table></section><section className="official-report-grid"><div><h2>B. Kehadiran</h2><dl>{(["Hadir","Terlambat","Sakit","Izin","Alpa"] as Attendance[]).map(status=><div key={status}><dt>{status}</dt><dd>{attendance.filter(item=>item.status===status).length}</dd></div>)}</dl></div><div><h2>C. Perkembangan & Karakter</h2><dl>{(["Portofolio","Prestasi","Pelanggaran"] as DevelopmentType[]).map(type=><div key={type}><dt>{type}</dt><dd>{records.filter(item=>item.type===type).length} catatan · {records.filter(item=>item.type===type).reduce((sum,item)=>sum+item.points,0)} poin</dd></div>)}</dl></div></section><section className="official-notes"><h2>D. Catatan Wali Kelas</h2><p>{average>=85&&attendanceRate>=80?"Ananda menunjukkan capaian akademik dan kedisiplinan yang sangat baik. Pertahankan konsistensi serta terus kembangkan karya dan prestasi.":average>=75?"Ananda menunjukkan perkembangan yang baik. Tingkatkan konsistensi belajar, kehadiran, dan tanggung jawab dalam menyelesaikan tugas.":"Ananda memerlukan pendampingan lebih lanjut dalam belajar dan kedisiplinan. Kerja sama sekolah dan wali santri sangat diperlukan."}</p></section><footer><div className="report-verification"><ReportVerificationQr value={verification}/><span><strong>QR Verifikasi</strong><small>{verification}</small></span></div><div className="report-signature"><span>{formatIndonesianDate(todayIso())}</span><small>Wali Kelas</small><strong>{data.settings?.homeroom??data.actor.name}</strong><em>✓ Ditandatangani secara digital</em></div></footer></article></>;
}

function AdministrationPage({ activeClass, data, refresh }: { activeClass:(typeof classOptions)[number];data:BootstrapData;refresh:()=>Promise<void> }) {
  const isAdmin=data.actor.role==="Admin";const tabs=isAdmin?["Pengguna","Semester","Laporan Semester","Rekap Absensi","Pengumpulan","Portal Siswa","Kenaikan & Data","Persetujuan","Komunikasi","Pemulihan"]:["Laporan Semester","Rekap Absensi","Pengumpulan","Komunikasi"];
  const [tab,setTab]=useState(tabs[0]);const [selectedNis,setSelectedNis]=useState(data.students[0]?.nis??"");const [selectedStudents,setSelectedStudents]=useState<string[]>([]);
  const [editingUser,setEditingUser]=useState<UserRow|null>(null);
  const request=async(resource:string,payload:Record<string,unknown>,operation="create")=>{const response=await fetch("/api/sikelas",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({resource,operation,data:payload})});const result=await response.json() as {error?:string};if(!response.ok)throw new Error(result.error||"Operasi gagal.");await refresh();return result};
  const activePeriod=data.periods.find(item=>item.active);
  const attendanceSummary=data.students.map(student=>{const rows=data.attendanceHistory.filter(item=>item.nis===student.nis);const present=rows.filter(item=>item.status==="Hadir"||item.status==="Terlambat").length;return{student,total:rows.length,present,sick:rows.filter(item=>item.status==="Sakit").length,permit:rows.filter(item=>item.status==="Izin").length,absent:rows.filter(item=>item.status==="Alpa").length,rate:rows.length?Math.round(present/rows.length*100):0}});
  const portalStudent=data.students.find(item=>item.nis===selectedNis)??data.students[0];const portalGrade=data.grades.find(item=>item.nis===selectedNis);const portalAttendance=attendanceSummary.find(item=>item.student.nis===selectedNis);const portalWorks=data.portfolios.filter(item=>item.nis===selectedNis);const portalSubmissions=data.submissions.filter(item=>item.nis===selectedNis);
  const createPeriod=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);await request("period",{label:form.get("label"),semester:form.get("semester"),startDate:form.get("startDate"),endDate:form.get("endDate")})};
  const saveSubmission=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);await request("submission",{taskId:form.get("taskId"),nis:form.get("nis"),status:form.get("status"),score:form.get("score"),feedback:form.get("feedback"),evidenceUrl:form.get("evidenceUrl")})};
  const promote=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);const target=String(form.get("target"));if(!window.confirm(`Pindahkan ${selectedStudents.length} siswa ke ${target}?`))return;await request("promoteStudents",{from:activeClass.label,to:target,nis:selectedStudents});setSelectedStudents([])};
  const importCsv=async(event:React.ChangeEvent<HTMLInputElement>)=>{const file=event.target.files?.[0];if(!file)return;const lines=(await file.text()).split(/\r?\n/).filter(Boolean);const headers=lines.shift()?.split(",").map(item=>item.trim().toLowerCase())??[];const rows=lines.map(line=>{const values=line.split(",").map(item=>item.trim());return Object.fromEntries(headers.map((key,index)=>[key,values[index]??""]))});await request("importStudents",{classLabel:activeClass.label,rows})};
  const communicate=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);const nis=String(form.get("nis")),channel=String(form.get("channel")),message=String(form.get("message"));const student=data.students.find(item=>item.nis===nis);await request("communication",{nis,channel,message});if(channel==="WhatsApp"&&student)window.open(`https://wa.me/62${student.guardian.replace(/\D/g,"").replace(/^0/,"")}?text=${encodeURIComponent(message)}`,"_blank");else window.location.href=`mailto:?subject=${encodeURIComponent(`Informasi ${student?.name??nis}`)}&body=${encodeURIComponent(message)}`};
  const saveUser=async(event:React.FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);await request("userManagement",{email:form.get("email"),name:form.get("name"),role:form.get("role"),status:form.get("status"),classLabels:form.getAll("classLabels"),subjectAssignments:form.getAll("subjectAssignments")});setEditingUser(null)};
  const updateUserStatus=(user:UserRow,status:string)=>request("userManagement",{email:user.email,name:user.name,role:user.role,status,classLabels:(user.assignedClasses||user.classLabel||"").split(",").filter(Boolean),subjectAssignments:(user.assignedSubjects||"").split(",").filter(Boolean)});
  return <><div className="admin-tabs">{tabs.map(item=><button className={tab===item?"active":""} onClick={()=>setTab(item)} key={item}>{item}</button>)}</div>
    {tab==="Laporan Semester"&&<SemesterReportPanel data={data} activeClass={activeClass} selectedNis={selectedNis} onSelect={setSelectedNis}/>}
    {tab==="Pengguna"&&isAdmin&&<div className="admin-grid user-management"><form key={editingUser?.email??"new-user"} className="panel inline-form" onSubmit={saveUser}><div className="toolbar"><div><h2>{editingUser?"Edit Pengguna":"Tambah Guru / Pengguna"}</h2><p className="muted">Akun aktif dapat masuk sesuai peran dan kelas yang dipilih.</p></div>{editingUser&&<button type="button" className="secondary-button" onClick={()=>setEditingUser(null)}>Batal Edit</button>}</div><div className="form-grid"><label>Nama lengkap<input name="name" required defaultValue={editingUser?.name??""}/></label><label>Email akun ChatGPT<input name="email" type="email" required readOnly={Boolean(editingUser)} defaultValue={editingUser?.email??""}/></label><label>Peran<select name="role" defaultValue={editingUser?.role??"Guru"}><option>Admin</option><option>Wali Kelas</option><option>Guru</option></select></label><label>Status<select name="status" defaultValue={editingUser?.status??"Active"}><option value="Active">Aktif</option><option value="Pending">Menunggu Persetujuan</option><option value="Nonaktif">Nonaktif</option></select></label></div><fieldset className="class-assignment"><legend>Penugasan kelas</legend>{data.classes.map(item=><label key={item.label}><input type="checkbox" name="classLabels" value={item.label} defaultChecked={(editingUser?.assignedClasses??editingUser?.classLabel??"").split(",").includes(item.label)}/><span>{item.label}<small>{item.students} siswa</small></span></label>)}</fieldset><fieldset className="class-assignment subject-assignment"><legend>Penugasan mata pelajaran</legend>{data.subjectCatalog.map(item=>{const value=`${item.classLabel}::${item.subject}`;return <label key={value}><input type="checkbox" name="subjectAssignments" value={value} defaultChecked={(editingUser?.assignedSubjects??"").split(",").includes(value)}/><span>{item.subject}<small>{item.classLabel}</small></span></label>})}</fieldset><button className="primary-button">{editingUser?"Simpan Perubahan":"Tambahkan Pengguna"}</button></form><article className="panel approval-list user-list"><div className="toolbar"><div><h2>Daftar Pengguna</h2><p className="muted">{data.users.filter(item=>item.status==="Active").length} aktif · {data.users.filter(item=>item.status==="Pending").length} menunggu</p></div><span className="settings-tag">Admin Pusat</span></div>{data.users.map(user=><div key={user.email}><span><strong>{user.name}</strong><small>{user.email}</small><small>{user.role} · {user.assignedClasses||user.classLabel||"Tanpa kelas"}</small><small>{user.assignedSubjects?`${user.assignedSubjects.split(",").length} penugasan mapel`:"Belum ada mapel"}</small></span><div><span className={`user-status ${user.status.toLowerCase()}`}>{user.status}</span>{user.status==="Pending"&&<button className="primary-button" onClick={()=>updateUserStatus(user,"Active")}>Setujui</button>}<button className="secondary-button" onClick={()=>setEditingUser(user)}>Edit</button>{user.email!==data.actor.email&&<button className="danger-button" onClick={()=>updateUserStatus(user,user.status==="Nonaktif"?"Active":"Nonaktif")}>{user.status==="Nonaktif"?"Aktifkan":"Nonaktifkan"}</button>}</div></div>)}</article></div>}
    {tab==="Semester"&&<div className="admin-grid"><article className="panel admin-card"><h2>Periode Aktif</h2>{activePeriod?<><strong className="big-value">{activePeriod.label} · {activePeriod.semester}</strong><p>{formatIndonesianDate(activePeriod.startDate)} – {formatIndonesianDate(activePeriod.endDate)}</p><button className="danger-button" onClick={()=>window.confirm("Tutup semester aktif dan kunci periodenya?")&&request("period",{id:activePeriod.id,label:activePeriod.label,semester:activePeriod.semester},"close")}>Tutup Semester</button></>:<p>Belum ada semester aktif.</p>}</article><form className="panel inline-form" onSubmit={createPeriod}><h2>Buka Semester Baru</h2><div className="form-grid"><label>Tahun ajaran<input name="label" required placeholder="2027/2028"/></label><label>Semester<select name="semester"><option>Ganjil</option><option>Genap</option></select></label><label>Tanggal mulai<input name="startDate" type="date" required/></label><label>Tanggal selesai<input name="endDate" type="date" required/></label></div><button className="primary-button">Aktifkan Semester</button></form></div>}
    {tab==="Rekap Absensi"&&<article className="panel data-panel"><div className="toolbar"><div><h2>Rekap Kehadiran Bulan Berjalan</h2><p className="muted">{data.attendanceHistory.length} catatan · {activeClass.label}</p></div><div className="toolbar-actions"><button className="secondary-button" onClick={()=>downloadCsv(`rekap-absensi-${activeClass.label}.csv`,[["NIS","Nama","Dicatat","Hadir","Sakit","Izin","Alpa","Persentase"],...attendanceSummary.map(item=>[item.student.nis,item.student.name,item.total,item.present,item.sick,item.permit,item.absent,`${item.rate}%`])])}>⇩ CSV</button><button className="secondary-button" onClick={()=>window.print()}>⎙ PDF</button></div></div><div className="table-scroll"><table><thead><tr><th>Siswa</th><th>Dicatat</th><th>Hadir</th><th>Sakit</th><th>Izin</th><th>Alpa</th><th>Kehadiran</th></tr></thead><tbody>{attendanceSummary.map(item=><tr key={item.student.nis}><td><strong>{item.student.name}</strong><small>{item.student.nis}</small></td><td>{item.total}</td><td>{item.present}</td><td>{item.sick}</td><td>{item.permit}</td><td>{item.absent}</td><td><span className={`predicate ${item.rate<80?"warning":""}`}>{item.rate}%</span></td></tr>)}</tbody></table></div></article>}
    {tab==="Pengumpulan"&&<div className="admin-grid"><form className="panel inline-form" onSubmit={saveSubmission}><h2>Catat Pengumpulan & Nilai</h2><div className="form-grid"><label>Tugas<select name="taskId">{data.tasks.filter(item=>item.id).map(item=><option value={item.id} key={item.id}>{item.title}</option>)}</select></label><label>Siswa<select name="nis">{data.students.map(item=><option value={item.nis} key={item.nis}>{item.name}</option>)}</select></label><label>Status<select name="status"><option>Dikumpulkan</option><option>Terlambat</option><option>Perlu Revisi</option><option>Dinilai</option></select></label><label>Nilai<input name="score" type="number" min="0" max="100"/></label></div><label>Tautan jawaban<input name="evidenceUrl" type="url" placeholder="https://..."/></label><label>Umpan balik<textarea name="feedback" rows={3}/></label><button className="primary-button">Simpan Pengumpulan</button></form><article className="panel compact-list"><h2>Riwayat Pengumpulan</h2>{data.submissions.map(item=><div key={item.id}><span><strong>{item.taskTitle}</strong><small>{data.students.find(student=>student.nis===item.nis)?.name??item.nis}</small></span><b>{item.score??item.status}</b></div>)}</article></div>}
    {tab==="Portal Siswa"&&<div className="admin-grid"><article className="panel portal-preview"><div className="toolbar"><div><h2>Portal Siswa & Wali Santri</h2><p className="muted">Data dibatasi untuk satu siswa.</p></div><select value={selectedNis} onChange={event=>setSelectedNis(event.target.value)}>{data.students.map(item=><option value={item.nis} key={item.nis}>{item.name}</option>)}</select></div>{portalStudent&&<><header><span className={`avatar ${portalStudent.color}`}>{portalStudent.initials}</span><div><h2>{portalStudent.name}</h2><p>NIS {portalStudent.nis} · {activeClass.label}</p></div><a href={`https://wa.me/62${portalStudent.guardian.replace(/\D/g,"").replace(/^0/,"")}`} target="_blank" rel="noreferrer" className="secondary-button">Hubungi Wali</a></header><section className="portal-stats"><div><strong>{portalAttendance?.rate??0}%</strong><small>Kehadiran</small></div><div><strong>{portalGrade?.finalScore??0}</strong><small>Nilai akhir</small></div><div><strong>{portalSubmissions.length}</strong><small>Tugas terkumpul</small></div><div><strong>{portalWorks.length}</strong><small>Portofolio</small></div></section><p className="portal-note">Saat akun ini masuk, server hanya mengirim data siswa yang dipetakan.</p></>}</article><form className="panel inline-form" onSubmit={async event=>{event.preventDefault();const form=new FormData(event.currentTarget);await request("accountMapping",{email:form.get("email"),role:form.get("role"),nis:form.get("nis")});window.alert("Akun portal berhasil dipetakan.")}}><h2>Hubungkan Akun Portal</h2><label>Email akun ChatGPT<input name="email" type="email" required placeholder="wali@email.com"/></label><label>Peran<select name="role"><option>Siswa</option><option>Wali Santri</option></select></label><label>Siswa<select name="nis">{data.students.map(item=><option value={item.nis} key={item.nis}>{item.name} · {item.nis}</option>)}</select></label><button className="primary-button">Simpan Akses Portal</button></form></div>}
    {tab==="Kenaikan & Data"&&<div className="admin-grid"><form className="panel inline-form" onSubmit={promote}><h2>Kenaikan / Pindah Kelas</h2><div className="select-students">{data.students.map(item=><label key={item.nis}><input type="checkbox" checked={selectedStudents.includes(item.nis)} onChange={event=>setSelectedStudents(current=>event.target.checked?[...current,item.nis]:current.filter(nis=>nis!==item.nis))}/><span>{item.name}<small>{item.nis}</small></span></label>)}</div><label>Kelas tujuan<select name="target">{classOptions.filter(item=>item.label!==activeClass.label).map(item=><option key={item.label}>{item.label}</option>)}</select></label><button className="primary-button" disabled={!selectedStudents.length}>Pindahkan {selectedStudents.length} Siswa</button></form><article className="panel admin-card"><h2>Impor & Ekspor Data</h2><p>Format CSV: <code>nis,name,gender,guardian</code></p><label className="upload-button">⇧ Impor CSV<input type="file" accept=".csv,text/csv" onChange={importCsv}/></label><button className="secondary-button wide" onClick={()=>downloadCsv(`data-siswa-${activeClass.label}.csv`,[["nis","name","gender","guardian","classLabel"],...data.students.map(item=>[item.nis,item.name,item.gender,item.guardian,activeClass.label])])}>⇩ Ekspor Siswa</button></article></div>}
    {tab==="Persetujuan"&&<article className="panel approval-list"><h2>Persetujuan Portofolio</h2>{data.portfolios.map(item=><div key={item.id??item.title}><span><strong>{item.title}</strong><small>{item.student} · Status: {item.status}</small></span><div><button className="secondary-button" onClick={()=>item.id&&request("portfolioApproval",{id:item.id,status:"Disetujui"})}>Setujui</button><button className="danger-button" onClick={()=>item.id&&request("portfolioApproval",{id:item.id,status:"Ditolak"})}>Tolak</button></div></div>)}</article>}
    {tab==="Komunikasi"&&<div className="admin-grid"><form className="panel inline-form" onSubmit={communicate}><h2>Pesan untuk Wali Santri</h2><div className="form-grid"><label>Siswa<select name="nis">{data.students.map(item=><option value={item.nis} key={item.nis}>{item.name}</option>)}</select></label><label>Saluran<select name="channel"><option>WhatsApp</option><option>Email</option></select></label></div><label>Pesan<textarea name="message" required rows={5} placeholder="Tuliskan informasi absensi, tugas, atau perkembangan siswa..."/></label><button className="primary-button">Catat & Buka Pengiriman</button></form><article className="panel compact-list"><h2>Riwayat Komunikasi</h2>{data.communications.map(item=><div key={item.id}><span><strong>{item.student??item.nis}</strong><small>{item.channel} · {new Date(item.createdAt).toLocaleString("id-ID")}</small></span><b>{item.status}</b></div>)}</article></div>}
    {tab==="Pemulihan"&&<article className="panel approval-list"><h2>Data Terhapus</h2>{data.deletedRecords.length?data.deletedRecords.map(item=><div key={item.id}><span><strong>{item.entity} · {item.entityId}</strong><small>Dihapus {new Date(item.deletedAt).toLocaleString("id-ID")} oleh {item.deletedBy}</small></span><button className="secondary-button" onClick={()=>request("restore",{id:item.id})}>↻ Pulihkan</button></div>):<div className="empty-state"><strong>Tidak ada data di tempat pemulihan.</strong></div>}</article>}
  </>;
}
...
export default function Home() {
  ...
  const [accessState,setAccessState]=useState<{loading:boolean;error:string|null}>({loading:true,error:null});
  const [weather,setWeather]=useState<{temperature:number|null;description:string;location:string}>({temperature:null,description:"Memuat cuaca...",location:"Pesawaran, Lampung"});
  const refreshData = useCallback(async () => { const response = await fetch(`/api/sikelas?class=${encodeURIComponent(activeClass.label)}&date=${todayIso()}`); const payload = await response.json() as BootstrapData & { error?: string }; if (!response.ok) throw new Error(payload.error ?? "Gagal memuat data."); setAppData(payload);setAccessState({loading:false,error:null});const resolved=classOptions.find(item=>item.label===payload.classLabel);if(resolved&&resolved.label!==activeClass.label)setActiveClass(resolved); }, [activeClass.label]);
  useEffect(() => {
    let active = true;
    fetch(`/api/sikelas?class=${encodeURIComponent(activeClass.label)}&date=${todayIso()}`)
      .then(async (response) => {
        ...
        if (active){setAppData(payload);setAccessState({loading:false,error:null});const resolved=classOptions.find(item=>item.label===payload.classLabel);if(resolved&&resolved.label!==activeClass.label)setActiveClass(resolved)}
      })
      .catch((error:unknown) => { if (active){setAppData(null);setAccessState({loading:false,error:error instanceof Error?error.message:"Akses aplikasi ditolak."})} });
    return () => { active = false; };
  }, [activeClass.label]);
  ...
  useEffect(()=>{const closeOutside=(event:PointerEvent)=>{if(!classSwitcherRef.current?.contains(event.target as Node))setClassMenuOpen(false)};const closeWithEscape=(event:KeyboardEvent)=>{if(event.key!=="Escape")return;setClassMenuOpen(false);setMenuOpen(false);setNotifications(false);setProfileOpen(false);setSidebarProfileOpen(false)};document.addEventListener("pointerdown",closeOutside);document.addEventListener("keydown",closeWithEscape);return()=>{document.removeEventListener("pointerdown",closeOutside);document.removeEventListener("keydown",closeWithEscape)}},[]);
  ...
  const isPortalViewer=["Siswa","Wali Santri"].includes(appData?.actor.role??"");const visibleNavItems=isPortalViewer?navItems.filter(item=>["dashboard","tugas","perkembangan"].includes(item.key)):navItems;const visibleSecondaryItems=isPortalViewer?secondaryNavItems.filter(item=>item.key==="portofolio"):appData?.actor.role==="Guru"?secondaryNavItems.filter(item=>item.key!=="pengaturan"):secondaryNavItems;
  const visibleClassOptions=appData?.classes.map(item=>classOptions.find(option=>option.label===item.label)).filter((item):item is (typeof classOptions)[number]=>Boolean(item))??[];
  ...
  if(accessState.loading)return <div className="access-page"><div className="access-card"><Logo/><p>SIKELAS NURUL IMAN</p><h1>Menyiapkan ruang kerja...</h1><span>Memeriksa akun dan penugasan kelas Anda.</span></div></div>;
  if(accessState.error)return <div className="access-page"><div className="access-card denied"><Logo/><p>AKSES AKUN</p><h1>Akun belum dapat digunakan</h1><span>{accessState.error}</span><small>Hubungi Admin Pusat untuk persetujuan dan penugasan kelas.</small><a className="primary-button" href="/signout-with-chatgpt?return_to=/">Keluar & ganti akun</a></div></div>;

  return (
    <div className="app-shell">
      <aside id="main-sidebar" className={menuOpen ? "sidebar open" : "sidebar"}>
        <div className="brand"><button type="button" className="brand-home" onClick={()=>{setPage("dashboard");setMenuOpen(false);setClassMenuOpen(false)}} aria-label="Kembali ke Dashboard"><Logo /><span className="brand-copy"><strong>SIKELAS</strong><span>NURUL IMAN</span></span></button><button type="button" className="mobile-close" onClick={() => setMenuOpen(false)} aria-label="Tutup menu">×</button></div>
        <div className="class-switcher" ref={classSwitcherRef}>
          <button type="button" className="class-selector" onClick={() => setClassMenuOpen(value=>!value)} aria-expanded={classMenuOpen} aria-haspopup="menu" aria-controls="class-menu">
          ...
          <div id="class-menu" className={classMenuOpen ? "class-menu open" : "class-menu"} role="menu" aria-label="Pilih kelas">
            {visibleClassOptions.map((classItem) => <button type="button" role="menuitem" className={activeClass.label === classItem.label ? "active" : ""} key={classItem.label} onClick={() => { setActiveClass(classItem); setClassMenuOpen(false); }}><span>{classItem.short}</span><span><strong>{classItem.label}</strong><small>{classItem.students} siswa</small></span>{activeClass.label === classItem.label && <b>✓</b>}</button>)}
          </div>
        </div>
        <nav aria-label="Menu utama"><small>MENU UTAMA</small>{visibleNavItems.map((item) => <button className={page === item.key ? "active" : ""} key={item.key} onClick={() => { setPage(item.key); setMenuOpen(false); }}><span>{item.icon}</span>{item.label}{item.key === "tugas" && <i>3</i>}</button>)}</nav>
        <nav className="secondary-nav" aria-label="Menu lainnya"><small>LAINNYA</small>{visibleSecondaryItems.map((item) => <button className={page === item.key ? "active" : ""} key={item.key} onClick={() => { setPage(item.key); setMenuOpen(false); }}><span>{item.icon}</span>{item.label}{item.key === "pengumuman" && <i>3</i>}</button>)}</nav>
        <div className="sidebar-footer">
          ...
          {sidebarProfileOpen && <div className="sidebar-profile-menu" role="menu">
            ...
            {!isPortalViewer&&<button role="menuitem" onClick={() => { setPage("pengaturan"); setSidebarProfileOpen(false); setMenuOpen(false); }}>⚙ Pengaturan akun</button>}
          </div>}
      ...
      <main className="main-content">
        ...
        <div className="page-wrap">
          ...
          {page === "tugas" && <TasksPage activeClass={activeClass} studentRows={studentRows} taskRows={appData?.tasks ?? tasks} subjectGradeRows={appData?.subjectGrades??[]} subjects={appData?.subjects??[]} actor={appData?.actor??{email:"",name:"",role:"Guru",classLabel:activeClass.label}} refresh={refreshData} />}
          {page === "perkembangan" && <StudentDevelopmentPage key={activeClass.label} activeClass={activeClass.label} studentRows={studentRows} pointRules={appData?.pointRules ?? []} auditLogs={appData?.auditLogs ?? []} refresh={refreshData} settings={appData?.settings??null} actor={appData?.actor??{email:"guru@nuruliman.sch.id",name:"Abdurohman Yusuf",role:"Guru",classLabel:activeClass.label}} />}
          ...
          {page === "administrasi" && (appData?<AdministrationPage activeClass={activeClass} data={appData} refresh={refreshData}/>:<div className="empty-state"><strong>Memuat administrasi...</strong></div>)}
